...
Proper::Proper(Fraction frac)
{
	setNum(frac.getNum());
	setDen(frac.getDen());
}

void Proper::operator=(Fraction fract)
{
	setNum(fract.getNum());
	setDen(fract.getDen());
}
...
{
	if (getNum() < getDen())
		std::cout << getNum() << "/" << getDen() << endl;
	else if (getNum() % getDen() == 0)
		std::cout << getNum() / getDen() << endl;
	else
		std::cout << getNum() / getDen() << " & " << getNum() % getDen() << "/" << getDen() << endl;
}
